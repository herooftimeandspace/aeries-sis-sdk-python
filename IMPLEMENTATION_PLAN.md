...
- Client config is `base_url`, `certificate`, `default_database_year=None`, `timeout=30.0`, `user_agent`, and optional injected `httpx` transport or session.
...
## Test Plan
- Unit tests:
  - base URL normalization, header injection, certificate redaction, `DatabaseYear` merging, timeout handling, retry behavior, and sync/async parity
